...
{
    public partial class ProductionMethodsForm : Form
    {
        public List<ClassProductionMethods> ProductionMethodsListP;
        public ClassProductionMethods local;  // null if new   information if to change
        ...
        public ClassProductionMethods ReturnValue()
        {
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                form.currentMode = "Production Methods";
            ...
            {
                local = new ClassProductionMethods(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName);
                LoadInfoToControls();
            }
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(ProductionMethodsListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassProductionMethods(NameTB.Texts, NameGameTB.Texts,(local.Code==null)? string.Empty:local.Code);
        }
        ...
        private void TechForm_Load(object sender, EventArgs e)
        {
            ...
            {
                local = new ClassProductionMethods();
                SaveStatus = 2;
            }
        }
