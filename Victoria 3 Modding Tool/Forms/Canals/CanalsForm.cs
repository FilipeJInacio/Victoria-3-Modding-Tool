...
namespace Victoria_3_Modding_Tool
{
    ...
    {
        public List<ClassCanals> CanalsListP;
        public ClassCanals local;  // null if new   information if to change
        ...
        public ClassCanals ReturnValue()
        {
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                local = new ClassCanals(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName);
                LoadInfoToControls();
            }
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(CanalsListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassCanals(NameTB.Texts, NameGameTB.Texts,(local.Code==null)? string.Empty:local.Code);
        }
