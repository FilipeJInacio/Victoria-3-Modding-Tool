...
namespace Victoria_3_Modding_Tool
{
    public partial class TechForm : Form
    {
        ...
        public List<ClassTech> TechListP;
        public List<ClassModifiersType> ModifiersTypes; // Needed

        public List<ClassEras> EraList; // Needed   (Victoria 3 + Project)
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                form.currentMode = "Technologies";
                s = local.Name + " = {\n" +
                    "\tera = era_" + local.Era + "\n" +
                    "\ttexture = \"" + local.Texture + "\"\n" +
                    "\tcategory = " + local.Category + "\n";
                if (local.CanResearch == false) { s += "\tcan_research = no\n"; }
                ...
                if (local.Restrictions.Count != 0)
                {
                    s += "\n\tunlocking_technologies = {\n";
                    foreach (string restr in local.Restrictions)
                    {
                        s += "\t\t" + restr + "\n";
                    }
                    s += "\t}\n";
            ...
            if (s != string.Empty)
            {
                local = new ClassTech(((List<KeyValuePair<string, object>>)(new Parser().ParseText(s)))[0], local.TrueName, local.Desc);
                LoadInfoToControls();
            }
            SaveStatus = 2;
            this.Show();
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(TechListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        private void Save_Technology()
        {
            ...
            {
                local.Restrictions = new List<string>();
            }
        ...
        {
            foreach (ClassEras Era in EraList)
            {
