...
{
    public partial class DecreesForm : Form
    {
        public List<ClassDecrees> DecreesListP;

        public ClassDecrees local;  // null if new   information if to change
        ...
        public ClassDecrees ReturnValue()
        {
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                local = new ClassDecrees(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName, local.Description);
                LoadInfoToControls();
            }
            SaveStatus = 2;
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(DecreesListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassDecrees(NameTB.Texts, NameGameTB.Texts, DescriptionTB.Texts,(local.Code==null)? string.Empty:local.Code);
        }
