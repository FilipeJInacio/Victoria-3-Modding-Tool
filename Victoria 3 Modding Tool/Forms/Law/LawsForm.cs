...
{
    public partial class LawsForm : Form
    {
        public List<ClassLaws> LawsListP; 
        public ClassLaws local;  // null if new   information if to change
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                local = new ClassLaws(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName, local.Description);
                LoadInfoToControls();
            }
            SaveStatus = 2;
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(LawsListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassLaws(NameTB.Texts, NameGameTB.Texts, DescriptionTB.Texts,(local.Code==null)? string.Empty:local.Code);
        }
