...
{
    public partial class LawGroupsForm : Form
    {
        public List<ClassLawGroups> LawGroupsListP;
        public ClassLawGroups local;  // null if new   information if to change

        public bool[] canSave = { false, false, false}; // Name - TrueName - Desc   false -> cant save
        public int SaveStatus = 0;    // 0 -> opened just now   1 -> is saved   2 -> is not

        public LawGroupsForm()
        {
            InitializeComponent();
        ...
        public ClassLawGroups ReturnValue()
        {
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                local = new ClassLawGroups(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName, local.Description);
                LoadInfoToControls();
            }
            SaveStatus = 2;
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(LawGroupsListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassLawGroups(NameTB.Texts, NameGameTB.Texts, DescriptionTB.Texts,(local.Code==null)? string.Empty:local.Code);
        }
