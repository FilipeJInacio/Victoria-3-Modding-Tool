...
{
    public partial class BuildingGroupsForm : Form
    {
        public List<ClassBuildingGroups> BuildingGroupsListP;
        public ClassBuildingGroups local;  // null if new   information if to change
        ...
        public ClassBuildingGroups ReturnValue()
        {
        ...
        private void GoToCodeEditor()
        {
            ...
            {
                form.currentMode = "Building Groups";
            ...
            {
                local = new ClassBuildingGroups(((List<KeyValuePair<string, object>>)(new NoParse().ParseText(s)))[0], local.TrueName);
                LoadInfoToControls();
            }
        ...
        {
            if (!string.IsNullOrEmpty(NameTB.Texts) && !Functions.hasName(BuildingGroupsListP, NameTB.Texts) && Regex.Match(NameTB.Texts, "^([a-z]||_)+$").Success)
            {
                canSave[0] = true;
            }
        ...
        {
            local = new ClassBuildingGroups(NameTB.Texts, NameGameTB.Texts, (local.Code == null) ? string.Empty : local.Code);
        }
